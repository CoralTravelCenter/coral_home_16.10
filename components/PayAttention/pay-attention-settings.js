window.pay_attention_slider = [
	{
		headline: 'Популярные вопросы о покупке тура онлайн',
		text: '',
		action: {
			title: 'Узнать больше',
			go_to: '#'
		},
		bacground_img: './img/pay-attention-1.jpg',
	},
	{
		headline: 'Ваш помощник по туру',
		text: 'Официальное мобильное приложение Coral Travel',
		action: {
			title: 'Узнать больше',
			go_to: '#'
		},
		bacground_img: './img/pay-attention-2.jpg',
	},
	{
		headline: 'Отдых на виллах',
		text: 'Приватные виллы, где все продумано до мелочей',
		action: {
			title: 'Узнать больше',
			go_to: '#'
		},
		bacground_img: './img/pay-attention-3.jpg',
	},
	{
		headline: 'Умный подбор туров',
		text: '',
		action: {
			title: 'Найти тур',
			go_to: '#'
		},
		bacground_img: './img/pay-attention-4.jpg',
	}
]